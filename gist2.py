...
class hfUsers:
	...
	def login(self):
		# Trying to login but this script is broken
		r = self.session.get("https://hackforums.net/member.php?action=login")
		...
		rPOST = self.session.post("https://hackforums.net/member.php", data=payload)
		logoutkey = rPOST.text.split('&logoutkey="')[-1].split('";')[0]
		...
		#print('tid')
		logoutpayload = {'action':'logout' + '&' 'logoutkey=' + str(logoutkey)}
		print(logoutpayload)
		#rlogout = requests.get('https://hackforums.net/member.php', data=logoutpayload, headers=self.headers)
		print('sent request')
		mm.write_to_file("logout.html", rlogout.text)
		print('logout')

		if (rPOST.text.split('<td class="thead"><strong>')[-1].split('</strong></td>')[0]) == "Site Challenge":
			...
			r = self.session.post('http://hackforums.net/cdn-cgi/l/chk_captcha', data=payload)
			time.sleep(1)
			# Getting the main page again this time MAYBE without captcah since we solved it
			r = self.session.get('http://hackforums.net/')
			#Writing to a file
			text_file = open("output1.txt","w") # Writes the html to a text file
			text_file.write(str(r.text))
			text_file.close()
		print('Done')
